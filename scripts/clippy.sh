#!/usr/bin/env bash

cargo clippy --package clitinfo
cargo clippy --package countlines
cargo clippy --package installer
cargo clippy --package filers
cargo clippy --package listdirs
cargo clippy --package mimers
cargo clippy --package meta
cargo clippy --package music-meta-renamer
cargo clippy --package progressbar
cargo clippy --package rot
cargo clippy --package sysinfo
cargo clippy --package timers
cargo clippy --package treers
cargo clippy --package uninstaller
